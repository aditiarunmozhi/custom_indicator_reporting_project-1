...
lao7op <- lao_5_7 %>% filter(orgunit_level == 7) %>% select(orgunit_level:orgunit_name) %>% arrange(orgunit_name)
lao7uid <- c(lao7op$orgunit_uid)
lao7name <- unique(c(lao7op$orgunit_name))

lao6op <- lao_5_7 %>% filter(orgunit_level == 6) %>% select(orgunit_level:orgunit_name) %>% arrange(orgunit_name)
